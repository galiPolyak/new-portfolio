          ...
          <>
            ...
            <div className={`lg:hidden absolute top-24 right-0 bg-gradient-toggle-menu text-white w-full max-w-xs p-3 rounded shadow-lg transition-transform duration-300 ${isMenuActive ? 'translate-y-0 opacity-100 z-40' : '-translate-y-80 opacity-0 z-0'}`}>
              <div className="flex flex-col items-center">
                <button
                  onClick={() => setCurrentPage(PageStatus.HOME)}
                  className={`block py-4 text-center text-1xl hover:text-[rgb(149,4,118)] transition-colors duration-300 ${currentPage === PageStatus.HOME ? '' : ''}`}> {/* Changed text size */}
                  Home
                </button>
                <button
                  onClick={() => setCurrentPage(PageStatus.RES)}
                  className={`block py-4 text-center text-1xl hover:text-[rgb(149,4,118)] transition-colors duration-300 ${currentPage === PageStatus.RES ? '' : ''}`}> {/* Changed text size */}
                  Resume
                </button>
                <button
                  onClick={() => setCurrentPage(PageStatus.COOP)}
                  className={`block py-4 text-center text-1xl hover:text-[rgb(149,4,118)] transition-colors duration-300 ${currentPage === PageStatus.COOP ? '' : ''}`}> {/* Changed text size */}
                  Co-op
                </button>
                <a
                  href="https://github.com/galiPolyak/"
                  target="_blank"
                  className="text-3xl py-2 text-white hover:text-[rgb(149,4,118)] transition-colors duration-300 mt-4"
                >
